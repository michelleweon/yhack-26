...
const UPLOAD_OPTIONS: Array<{
  ...
  { sourceType: 'enemies', label: 'Upload List of Enemies' },
  { sourceType: 'trash', label: 'Upload Trash' },
...
export function MiscPackManager({ packs }: MiscPackManagerProps) {
  ...
  const [pendingSourceType, setPendingSourceType] = useState<CustomPackSourceType | null>(null);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!normalizedFileName.endsWith('.pdf') && !normalizedFileName.endsWith('.txt')) {
      setErrorMessage('Only PDF and TXT files are supported for misc scraps.');
      return;
    }
    ...
    try {
      ...
      } = await extractTextFromCustomPackFile(file);
      ...
      const nextError =
        error instanceof Error ? error.message : 'Unable to create a custom pack right now.';
  ...
  const handleDeletePack = async () => {
    ...
    setErrorMessage(null);
    setStatusMessage(`Deleting ${getDisplayName(pendingDeletePack.filename)}...`);
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Unable to delete that custom pack right now.');
      setStatusMessage(null);
    } finally {
          ...
          >
            ...
            >
              ...
              <p className="mt-3 font-ui text-lg leading-relaxed text-white/88">
                {getDisplayName(pendingEmptyDocumentConfirmation.file.name)} looks empty or has almost no extractable text.
              </p>
        ...
        {pendingDeletePack && (
          ...
          >
            <button
              type="button"
              className="absolute inset-0 bg-black/65"
              aria-label="Cancel delete custom pack"
              onClick={() => {
            ...
            >
              ...
              <p className="mt-3 font-ui text-lg leading-relaxed text-white/88">
                Delete <span className="font-semibold">{getDisplayName(pendingDeletePack.filename)}</span> from your saved packs?
              </p>
              <p className="mt-2 font-ui text-sm text-white/52">
